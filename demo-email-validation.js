...
class EmailValidationDemo {
  ...
  demonstrateValidation() {
    ...
    const validationResults = businessEmails.map(email => {
      const hasMxRecords = !email.includes('tempmail.org') && email.includes('@') && email !== 'invalid-email';
      return this.validateEmail(email, hasMxRecords);
    });

    console.log('Business Email Validation Results:');
    validationResults.forEach((result, index) => {
      console.log(`${index + 1}. ${result.email} - Valid: ${result.isValid}, Confidence: ${result.confidence}%, Role-based: ${result.isRoleBased}, Disposable: ${result.isDisposable}`);
    });
