...
test.describe('Search Engine Management E2E Tests', () => {
  ...
  test.describe('Search Engine Configuration', () => {
    test('should allow configuring search engines', async ({ page }) => {
      ...
      const settingsButton = page.locator('[data-testid="settings-button"], button:has-text("Settings"), [aria-label="Settings"]')
      
      if (await settingsButton.isVisible()) {
        await settingsButton.click()
        
        // Look for search engine configuration options
        const searchEngineSection = page.locator('[data-testid="search-engines"], .search-engines, :has-text("Search Engines")')
        ...
        if (await searchEngineSection.isVisible()) {
          ...
          if (checkboxCount > 0) {
            ...
            if (await saveButton.isVisible()) {
              ...
              await Promise.race([
                page.waitForSelector('[data-testid="save-success"], .success-message', { timeout: 5000 }),
    ...
    test('should handle search engine priority configuration', async ({ page }) => {
      ...
      const advancedButton = page.locator('[data-testid="advanced-options"], button:has-text("Advanced"), .advanced-toggle')
      
      if (await advancedButton.isVisible()) {
        await advancedButton.click()
        
        // Look for search engine selection
        const engineSelect = page.locator('select[name="searchEngine"], [data-testid="search-engine-select"]')
      ...
      await expect(page.locator('input[name="industry"], [data-testid="industry-input"]')).toBeVisible()
    })
  })
      ...
      await context.route('**/api/search**', async route => {
        ...
        if (url.includes('retry=true')) {
          await route.fulfill({
            ...
            body: JSON.stringify({
              results: [
                { id: '1', name: 'Fallback Business', url: 'https://example.com' }
              ]
            })
          })
        } else {
      ...
      await context.route('**/api/search**', async route => {
        ...
        if (requestCount === 1) {
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              results: [
                { id: '1', name: 'Retry Success Business', url: 'https://example.com' }
              ]
            })
          })
        }
  ...
  test.describe('Search Engine Rate Limiting', () => {
    test('should respect rate limits', async ({ page }) => {
      ...
      for (let i = 0; i < 5; i++) {
        searchPromises.push(
          performSearch(page, `business${i}`, `1234${i}`)
  ...
  test.describe('Search Engine Health Monitoring', () => {
    test('should detect unhealthy search engines', async ({ page, context }) => {
      ...
      const healthIndicator = page.locator('[data-testid="health-status"], .health-indicator, .status-indicator')
