  ...
  async (request: NextRequest, context) => {
    try {
      ...
      if (!isAdmin && context.user.id !== userId) {
        ...
        delete responseUser.lockedUntil
        delete responseUser.twoFactorSecret
        delete responseUser.preferences
      }
  ...
  async (request: NextRequest, context) => {
    try {
      ...
      if (permanent) {
        ...
        if (!context.user.roles?.some(role => role.role.name === 'admin')) {
          return NextResponse.json(
      ...
      const result = await context.database.query(query, [userId])
