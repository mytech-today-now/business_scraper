...
describe('Payment Validation Schemas', () => {
  ...
  describe('userSubscriptionSchema', () => {
    ...
    it('should reject when period end is before start', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.errors[0].message).toContain('Current period end must be after start date')
      }
    })
    ...
    it('should reject when updated date is before created date', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.errors[0].message).toContain('Updated date must be after or equal to created date')
      }
    })
