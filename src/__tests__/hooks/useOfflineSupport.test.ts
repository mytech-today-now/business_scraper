...
describe('useOfflineSupport', () => {
  ...
  it('should check connectivity with ping', async () => {
    ...
    const { result } = renderHook(() => useOfflineSupport({ pingUrl: '/api/test' }))
  ...
  it('should retry connection with exponential backoff', async () => {
    ...
    const { result } = renderHook(() => useOfflineSupport({ retryAttempts: 3, retryDelay: 10 }))
  ...
  it('should handle periodic connectivity checks', async () => {
    ...
    const { result } = renderHook(() => useOfflineSupport({ pingInterval: 1000 }))
  ...
  it('should not ping when interval is disabled', () => {
    ...
    renderHook(() => useOfflineSupport({ pingInterval: 0 }))
