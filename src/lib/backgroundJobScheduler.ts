...
export class BackgroundJobScheduler {
      ...
      for (const industry of industries) {
        try {
          ...
          logger.info(
            ...
            `Analyzed trends for ${industry}: ${trendAnalysis.trendDirection}`
          )
        } catch (error) {
    ...
    try {
      ...
      for (const job of jobsToProcess) {
        try {
          ...
          logger.info(
            'BackgroundJobScheduler',
            `Processed job ${job.id} for business ${business.businessName}`
          )
        } catch (error) {
  ...
  private async performDailyCleanup(): Promise<void> {
    try {
      ...
      logger.info(
        'BackgroundJobScheduler',
        `Daily cleanup completed: removed ${cleanedJobs} old jobs`
      )
    } catch (error) {
