...
export class EmailValidationService {
  ...
  private async performValidation(
    ...
    if (syntaxValid && mxRecords && !isDisposable) {
      try {
        ...
        logger.debug(
          ...
          `SMTP verification failed for ${normalizedEmail}`,
          error
        )
    ...
    try {
      ...
      logger.debug(
        'EmailValidationService',
        `Reputation scoring failed for ${normalizedEmail}`,
        error
      )
    }
  ...
  private async testSMTPConnection(
    ...
    return new Promise(resolve => {
      ...
      socket.on('data', data => {
        ...
        if (
          serverResponse.includes('greylist') ||
  ...
  private async initializeDisposableDomains(): Promise<void> {
    ...
    logger.debug(
      'EmailValidationService',
      `Initialized with ${this.disposableDomains.size} disposable domains`
    )
  }
