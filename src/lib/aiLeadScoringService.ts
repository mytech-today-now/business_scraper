...
export interface AIBadge {
  type: 'verified-email' | 'active-website' | 'complete-profile' | 'high-potential' | 
        'quick-responder' | 'established-business' | 'growth-industry' | 'local-leader'
  label: string
...
export interface AIWarning {
  type: 'incomplete-data' | 'outdated-info' | 'low-engagement' | 'competitive-market' | 'contact-difficulty'
  message: string
...
export class AILeadScoringService {
  ...
  async calculateBatchScores(businesses: BusinessRecord[]): Promise<AIScoreBatch> {
    ...
    try {
      ...
      for (let i = 0; i < businesses.length; i += batchSize) {
        ...
        batches.push(
          Promise.all(batch.map(async business => ({
            id: business.id,
      ...
      const averageScore = allScores.reduce((sum, score) => sum + score.overallScore, 0) / allScores.length
      
      const scoreDistribution = allScores.reduce((dist, score) => {
      ...
      logger.info('AILeadScoringService', `Processed ${businesses.length} businesses in ${processingTime}ms`)
  ...
  private scoreEstablishedPresence(business: BusinessRecord): number {
    ...
    const daysSinceScraped = Math.floor((Date.now() - business.scrapedAt.getTime()) / (1000 * 60 * 60 * 24))
