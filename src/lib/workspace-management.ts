...
export class WorkspaceManagementService {
   ...
   * Create a new team
   */
  static async createTeam(teamData: CreateTeamRequest, ownerId: string): Promise<Team> {
    try {
  ...
  static async addWorkspaceMember(
    ...
    try {
      ...
      const effectivePermissions =
        permissions.length > 0 ? permissions : RBACService.getWorkspaceRolePermissions(role)
      ...
      await database.query(
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `,
        [workspaceId, userId, role, JSON.stringify(effectivePermissions), invitedBy, new Date()]
      )
   ...
   */
  static async getWorkspaceMembership(
    workspaceId: string,
    userId: string
