...
export class EncryptionService {
  ...
  private initializeMasterKey(): void {
    ...
    try {
      ...
      if (this.masterKey.length !== KEY_LENGTH) {
        throw new Error(`Invalid master key length: expected ${KEY_LENGTH} bytes, got ${this.masterKey.length}`)
      }
   ...
   */
  public encryptWithPassword(data: string | Buffer, password: string, useScrypt = false): EncryptedData {
    try {
  ...
  public decryptWithPassword(encryptedData: EncryptedData, password: string): Buffer {
    ...
    try {
      ...
      const { key } = encryptedData.keyDerivation === 'scrypt'
...
export const encryptionService = {
  ...
  deriveKeyPBKDF2: (password: string, salt?: Buffer) => encryptionService.instance.deriveKeyPBKDF2(password, salt),
  deriveKeyScrypt: (password: string, salt?: Buffer) => encryptionService.instance.deriveKeyScrypt(password, salt),
  ...
  hash: (data: string | Buffer, algorithm = 'sha256') => encryptionService.instance.hash(data, algorithm),
