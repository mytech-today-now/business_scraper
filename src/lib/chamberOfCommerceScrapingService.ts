...
export class ChamberOfCommerceScrapingService {
   ...
   */
  async processChamberOfCommercePage(
    options: ChamberOfCommerceSearchOptions
  ): Promise<ChamberOfCommerceBusinessResult[]> {
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        const businessListings = await this.extractBusinessListingsFromChamberPage(
          page,
          maxBusinesses
        )
        ...
        logger.info(
          'COCPScraping',
          `Found ${businessListings.length} business listings on Chamber page`
        )

        // Extract business websites from Chamber profile pages
        const businessesWithWebsites = await this.extractBusinessWebsitesFromChamberProfiles(
          page,
          businessListings,
          maxBusinesses
        )
        ...
        const businessesWithDeepData = await this.performDeepWebsiteScraping(
        ...
        logger.info(
          'COCPScraping',
          `Successfully processed ${businessesWithDeepData.length} businesses from Chamber of Commerce`
        )
   ...
   */
  private async extractBusinessListingsFromChamberPage(
    page: Page,
    maxBusinesses: number
  ): Promise<any[]> {
    return await page.evaluate(maxBusinesses => {
      ...
      for (const selector of businessSelectors) {
        ...
        if (businessElements.length > 0) {
          console.log(
            `Found ${businessElements.length} business listings using selector: ${selector}`
          )
          break
        }
      }
      ...
      for (let i = 0; i < Math.min(businessElements.length, maxBusinesses); i++) {
        ...
        try {
          ...
          const fullProfileUrl = href.startsWith('/')
            ? `https://www.chamberofcommerce.com${href}`
            : href

          // Extract business name from the link content or nearby elements
          const businessName =
            businessElement.textContent?.trim() ||
          ...
          const address =
            businessContainer.querySelector('.address, .location')?.textContent?.trim() || ''
   ...
   */
  private async extractBusinessWebsitesFromChamberProfiles(
    page: Page,
    businesses: any[],
    maxBusinesses: number
  ): Promise<ChamberOfCommerceBusinessResult[]> {
    ...
    for (const business of businessesToProcess) {
      try {
        ...
        logger.info(
          'COCPScraping',
          `Extracting website from Chamber profile: ${business.chamberProfileUrl}`
        )
        ...
        await page.waitForSelector('main, .business-details, .profile, .content', {
          timeout: 10000,
        })
   ...
   * Perform deep scraping of business websites
   */
  private async performDeepWebsiteScraping(
    businesses: ChamberOfCommerceBusinessResult[],
    maxPagesPerSite: number
  ): Promise<ChamberOfCommerceBusinessResult[]> {
    ...
    for (const business of businesses) {
      try {
        ...
        if (scrapingResult && scrapingResult.length > 0) {
          ...
          logger.info(
            'COCPScraping',
            `Successfully scraped ${scrapingResult.length} business records from ${business.url}`
          )
        } else {
          ...
          logger.warn(
            'COCPScraping',
            `Deep scraping failed for ${business.url}, keeping basic info`
          )
        }
      } catch (error) {
    ...
    logger.info(
      'COCPScraping',
      `Completed deep scraping. ${businessesWithDeepData.length} businesses processed`
    )
    return businessesWithDeepData
  }
