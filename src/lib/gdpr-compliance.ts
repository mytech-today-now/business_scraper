...
export class GDPRComplianceService {
    ...
    try {
      ...
      switch (request.request_type) {
        ...
        case GDPRRequestType.PORTABILITY:
          responseData = await this.processPortabilityRequest(
            request.subject_email,
            requestDetails.preferredFormat
          )
          break
        case GDPRRequestType.RECTIFICATION:
          responseData = await this.processRectificationRequest(
            request.subject_email,
            requestDetails
          )
          break
  ...
  private getDataSubjectRights(): any {
    return {
      ...
      rightToErasure:
        'You have the right to obtain erasure of your personal data under certain circumstances.',
      rightToRestriction:
  ...
  private async sendVerificationEmail(
    ...
    logger.info(
      'GDPR',
      `Verification email would be sent to ${email} for ${requestType} request with token ${token}`
    )
  }
