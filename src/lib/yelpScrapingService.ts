...
export class YelpScrapingService {
    ...
    try {
      ...
      for (const selector of verificationSelectors) {
        ...
        if (element) {
          ...
          await page
            .waitForSelector('.search-results, [data-testid="search-results"], .searchResults', {
              timeout: 30000,
            })
            .catch(() => {
  ...
  async searchBusinesses(options: YelpSearchOptions): Promise<YelpBusinessResult[]> {
    ...
    logger.info(
      'YelpScraping',
      `Starting Yelp RESTful search: ${query} in ${location} (radius: ${zipRadius}mi)`
    )
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        await page.waitForSelector(
          '.search-results, [data-testid="search-results"], .searchResults',
          { timeout: 15000 }
        )
        ...
        const businessesWithWebsites = await this.extractBusinessWebsitesFromProfiles(
          page,
          businessListings,
          maxResults
        )
        ...
        logger.info(
          'YelpScraping',
          `Successfully processed ${filteredBusinesses.length} businesses with deep scraping`
        )
      ...
      for (let i = 0; i < Math.min(businessElements.length, maxResults); i++) {
        ...
        try {
          ...
          const businessContainer =
            businessElement.closest('[data-testid="search-result"]') ||
          ...
          if (businessContainer) {
            ...
            const addressElements = Array.from(
              businessContainer.querySelectorAll(
                '[data-testid="address"], .address, .business-address'
              )
            )
            ...
            const phoneElements = Array.from(
              businessContainer.querySelectorAll('[data-testid="phone"], .phone, .business-phone')
            )
   ...
   */
  private async extractBusinessWebsitesFromProfiles(
    page: Page,
    businesses: any[],
    maxResults: number
    ...
    for (const business of businessesToProcess) {
      try {
        ...
        await page.waitForSelector(
          '.business-website, [data-testid="business-website"], .biz-website',
          { timeout: 10000 }
        )
