...
export class BusinessIntelligenceService {
  ...
  private readonly socialMediaPatterns = [
    ...
    { platform: 'YouTube', patterns: ['youtube.com/channel/', 'youtube.com/c/', 'youtube.com/user/'] },
  ...
  public async enrichBusinessData(
    ...
    if (this.enrichmentCache.has(cacheKey)) {
      ...
      if (cached.companySize?.lastUpdated && 
  ...
  private async performBusinessEnrichment(
    ...
    try {
      ...
      const socialMediaPresence = await this.analyzeSocialMediaPresence(websiteUrl, businessName, page);
  ...
  private async estimateCompanySize(
    ...
    try {
      ...
      logger.debug('BusinessIntelligenceService', `Company size estimation failed for ${businessName}`, error);
      return {
        employeeRange: '1-10',
        confidence: 20,
  ...
  private getSizeRangeFromCategory(category: string): string {
    switch (category) {
      case 'small': return '1-10';
      case 'medium': return '11-50';
      case 'large': return '51-200';
  ...
  private analyzeBusinessNameForSize(businessName: string): { confidence: number; range?: string } {
    ...
    if (nameLower.includes('corporation') || nameLower.includes('corp') || 
        nameLower.includes('inc') || nameLower.includes('llc')) {
      return { confidence: 40, range: '11-50' };
    }
    
    // Small business indicators
    if (nameLower.includes('shop') || nameLower.includes('boutique') || 
        nameLower.includes('studio') || nameLower.includes('cafe')) {
  ...
  private async estimateRevenue(
    ...
    try {
      ...
      logger.debug('BusinessIntelligenceService', `Revenue estimation failed for ${businessName}`, error);
      return {
  ...
  private estimateRevenueFromSize(employeeRange: string): { range: string } {
    switch (employeeRange) {
      case '1-10': return { range: '$100K-$1M' };
      case '11-50': return { range: '$1M-$10M' };
      case '51-200': return { range: '$10M-$50M' };
      case '201-500': return { range: '$50M-$100M' };
      case '501-1000': return { range: '$100M-$500M' };
      case '1001-5000': return { range: '$500M-$1B' };
      case '5000+': return { range: '$1B+' };
      default: return { range: '$100K-$1M' };
    }
  }
  ...
  private getRevenueRangeFromLevel(level: string): string {
    switch (level) {
      case 'low': return '$100K-$1M';
      case 'medium': return '$1M-$10M';
      case 'high': return '$10M-$100M';
      case 'veryHigh': return '$100M+';
      default: return '$100K-$1M';
    }
  }
  ...
  private estimateRevenueFromIndustry(businessName: string): string {
    ...
    if (nameLower.includes('bank') || nameLower.includes('financial') ||
        nameLower.includes('insurance') || nameLower.includes('pharma')) {
      return '$10M-$100M';
    }

    // Medium-revenue industries
    if (nameLower.includes('tech') || nameLower.includes('software') ||
        nameLower.includes('consulting') || nameLower.includes('medical')) {
      return '$1M-$10M';
    }

    // Service industries
    if (nameLower.includes('restaurant') || nameLower.includes('retail') ||
        nameLower.includes('salon') || nameLower.includes('repair')) {
      return '$100K-$1M';
    }
  ...
  private async assessBusinessMaturity(
    ...
    try {
      ...
      logger.debug('BusinessIntelligenceService', `Maturity assessment failed for ${businessName}`, error);
      return {
  ...
  private analyzeBusinessNameForMaturity(businessName: string): {
    ...
    if (nameLower.includes('corp') || nameLower.includes('corporation') ||
    ...
    if (nameLower.includes('llc') || nameLower.includes('group') ||
    ...
    try {
      if (page) {
        ...
        const [htmlContent, scripts, stylesheets, headers] = await Promise.all([
          page.content(),
          page.evaluate(() => Array.from(document.querySelectorAll('script')).map(s => s.src || s.innerHTML)),
          page.evaluate(() => Array.from(document.querySelectorAll('link[rel="stylesheet"]')).map(l => l.href)),
          page.evaluate(() => {
      ...
      overallConfidence = Math.min(100, Math.max(0, overallConfidence / Math.max(1, platforms.length)));
      ...
      logger.debug('BusinessIntelligenceService', `Technology detection failed for ${websiteUrl}`, error);
      return {
        platforms: [],
        confidence: 0,
        lastScanned: timestamp
  ...
  private async analyzeSocialMediaPresence(
    ...
    try {
      ...
      logger.debug('BusinessIntelligenceService', `Social media analysis failed for ${websiteUrl}`, error);
      return {
        profiles: [],
        overallPresence: 0
      };
