...
async function testNormalMemoryUsage(page, baseUrl) {
  ...
  const avgMemory = measurements.reduce((sum, m) => sum + m.memory.usedJSHeapSize, 0) / measurements.length
  ...
  return {
    ...
    failureReason: maxMemory > MEMORY_THRESHOLDS.maxHeapSize ? 
      `Max memory ${Math.round(maxMemory / 1024 / 1024)}MB exceeds threshold ${Math.round(MEMORY_THRESHOLDS.maxHeapSize / 1024 / 1024)}MB` : null
  }
}
...
async function testMemoryLeaks(page, baseUrl) {
  ...
  return {
    ...
    failureReason: leakRate > MEMORY_THRESHOLDS.maxLeakRate ? 
      `Memory leak detected: ${Math.round(leakRate / 1024 / 1024)}MB per operation` : null
  }
}
...
async function testHeavyMemoryUsage(page, baseUrl) {
  ...
  return {
    ...
    failureReason: recoveryRate < 0.8 ? 
      `Poor memory recovery: only ${Math.round(recoveryRate * 100)}% recovered` : null
  }
}
...
async function testGarbageCollection(page, baseUrl) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(`  GC ${i + 1}: Collected ${Math.round((beforeGC.usedJSHeapSize - afterGC.usedJSHeapSize) / 1024 / 1024)}MB`)
  }
...
async function simulateUserInteractions(page) {
  ...
  await page.evaluate(() => {
    ...
    buttons.forEach((button, index) => {
      if (index < 3) { // Limit to first 3 buttons
        button.click()
      }
    })
...
async function generateMemoryReport(results) {
  ...
  fs.writeFileSync(
    path.join(reportDir, 'memory-report.json'),
